...
from analysis.data import get_train_datasets_stats, display_train
...
def compute_embeddings(train_dataset_stats, zeroshot_results):
    ...
    if not os.path.exists(
        "/vol/tmp/goldejon/gliner/eval_metric/zeroshot_embeddings.pkl"
    ):
        for i in tqdm(
        ...
        output_zeroshot.to_pickle(
            "/vol/tmp/goldejon/gliner/eval_metric/zeroshot_embeddings.pkl"
        )
    else:
        output_zeroshot = pd.read_pickle(
            "/vol/tmp/goldejon/gliner/eval_metric/zeroshot_embeddings.pkl"
        )

    if not os.path.exists(
        "/vol/tmp/goldejon/gliner/eval_metric/train_label_embeddings.pkl"
    ):
        for i in tqdm(
        ...
        output_train.to_pickle(
            "/vol/tmp/goldejon/gliner/eval_metric/train_label_embeddings.pkl"
        )
    else:
        output_train = pd.read_pickle(
            "/vol/tmp/goldejon/gliner/eval_metric/train_label_embeddings.pkl"
        )
...
def compute_distance(train_stats, train_embeddings, zeroshot_embeddings):
    if not os.path.exists("/vol/tmp/goldejon/gliner/eval_metric/distances.pkl"):
        distances = {
            "train_dataset": [],
            "eval_benchmark": [],
            "label": [],
        ...
        for _, row in tqdm(zeroshot_embeddings.iterrows(), desc="Compute Distance"):
            ...
            sim = cosine_similarity(z_emb, t_emb)
            top_k = sim.argsort()[0][::-1][:10]
        ...
        distances_df.to_pickle("/vol/tmp/goldejon/gliner/eval_metric/distances.pkl")
    else:
        distances_df = pd.read_pickle(
            "/vol/tmp/goldejon/gliner/eval_metric/distances.pkl"
        )
